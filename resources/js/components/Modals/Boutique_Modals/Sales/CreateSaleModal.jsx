...
import Modal from '../../Modal'; // Vérifiez que ce chemin est correct
import { useForm } from '@inertiajs/react';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { 
    faBarcode, faSearch, faTimes, faTrash, 
    faMoneyBillWave, faUser, faShoppingCart, faCheck 
...
const CreateSaleModal = ({ isOpen, onClose, products = [], customers = [], userCounterId }) => {
    ...
    // --- ÉTATS LOCAUX ---
    const [cart, setCart] = useState([]); // Le panier est géré localement par React
    const [searchQuery, setSearchQuery] = useState('');
    const [step, setStep] = useState(1); // 1: Sélection Produits, 2: Paiement
    const searchInputRef = useRef(null);

    // --- FORMULAIRE INERTIA ---
    // IMPORTANT : On déstructure 'transform' ici
    const { data, setData, post, processing, transform, reset } = useForm({
        customer_id: '',
        payment_mode: 'cash',
        received_amount: 0,
        items: [], // Sera rempli automatiquement par 'transform' avant l'envoi
        total_ht: 0,
        total_ttc: 0,
        counter_id: userCounterId || '', 
    });

    // --- OPTIONS CLIENTS (Pour React-Select) ---
    ...
    // 1. Filtrage Produits & Scan Code Barre
    ...
    // Détection automatique du scan (si résultat exact)
    useEffect(() => {
        const exactMatch = products.find(p => 
            p.sku === searchQuery || (p.barcode && p.barcode === searchQuery)
        );
        if (exactMatch) {
    ...
    const addToCart = (product) => {
        // Conversion explicite du prix (string -> float)
        ...
        // Utilisation de (?? 0) pour sécuriser si stock_comptoir est null
        if ((product.stock_comptoir ?? 0) <= currentInCart) {
            Swal.fire({
                icon: 'warning',
                title: 'Stock Insuffisant',
                text: `Il ne reste que ${product.stock_comptoir} unités au comptoir.`,
    ...
    const totals = useMemo(() => {
        ...
        const total_ttc = total_ht; // Adapter ici si la TVA s'ajoute en plus
        const total_tva = 0; 
        
        return { total_ht, total_tva, total_ttc };
    }, [cart]);

    // 4. Soumission AVEC TRANSFORM (Correction Critique)
    const handleSubmit = (e) => {
        ...
        // --- TRANSFORM : Injection des données dynamiques ---
        // Cette fonction s'exécute juste avant l'envoi de la requête POST.
        // Elle fusionne les données du formulaire (data) avec le panier (cart) et les totaux.
        transform((data) => ({
            ...data, // Garde customer_id, payment_mode, etc.
            items: cart.map(item => ({
        ...
        // Envoi de la requête (Inertia utilise automatiquement le résultat du transform)
        post(route('sales.store'), { 
            onSuccess: (page) => { 
                onClose();
                
                // Gestion de l'impression via l'URL renvoyée par le backend
            ...
            onError: (err) => {
                console.error(err);
                if (err.items) Swal.fire('Erreur', err.items, 'error'); // Erreur spécifique aux items
                else Swal.fire('Erreur', 'Vérifiez les données du formulaire.', 'error');
            }
        });
    };
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} maxWidth="full">
            <div className="flex h-[90vh] bg-gray-100 overflow-hidden font-sans">
                    ...
                    <div className="p-4 bg-white shadow-sm z-10 flex gap-4 items-center">
                        <div className="relative flex-1">
                            ...
                            <input
                                ...
                                placeholder="Scanner un article (Code-barre) ou rechercher..."
                                value={searchQuery}
                                ...
                                >
                                    <div className="h-32 w-full bg-gray-100 relative flex items-center justify-center overflow-hidden">
                                        ...
                                        <div className={`absolute bottom-2 left-2 text-[10px] font-bold px-1.5 py-0.5 rounded ${ (product.stock_comptoir ?? 0) > 5 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                                            Stock: {product.stock_comptoir ?? 0} {product.unit}
                                        </div>
                                    </div>
                ...
                <div className="w-1/3 flex flex-col bg-white shadow-2xl z-20 border-l border-gray-200">
                    ...
                    <div className="flex-1 overflow-y-auto p-3 space-y-2 bg-white">
                        {cart.length === 0 ? (
                            ...
                            cart.map((item) => (
                                <div key={item.product_id} className="flex items-center justify-between p-3 bg-white rounded-lg border border-gray-100 hover:border-blue-200 hover:shadow-sm transition-all group">
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <button onClick={() => removeFromCart(item.product_id)} className="text-gray-300 hover:text-red-500 transition-colors px-1">
                    ...
                    <div className="border-t border-gray-200 bg-gray-50 p-5 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
                        ...
                        {/* Modes : Bouton Payer vs Formulaire Paiement */}
                        {step === 1 ? (
                            <button
                                onClick={() => setStep(2)}
