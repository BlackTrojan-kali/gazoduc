...
import IndexOuvertureModal from '../../components/Modals/Fuel/IndexOuvertureModal'; // NOUVELLE MODALE
...
const MagFuelCiterne = ({ stocks, articles, agencies, cuvesFixes, pompes, citernesMobiles }) => {
    ...
    const [isIndexOuvertureModalOpen, setIsIndexOuvertureModalOpen] = useState(false);
    
    const [selectedStock, setSelectedStock] = useState(null);
    const [selectedPistolet, setSelectedPistolet] = useState(null); // Pour la nouvelle modale
    
    const { licence } = useLicenceChoice();

    // --- Gestion des Onglets ---
    const [activeTab, setActiveTab] = useState('cuves'); // 'cuves' ou 'pompes'
    ...
    // Nouvelle fonction pour ouvrir la modale d'initialisation des index
    const openIndexOuvertureModal = (pistolet) => {
        setSelectedPistolet(pistolet);
        setIsIndexOuvertureModalOpen(true);
    };
    const closeIndexOuvertureModal = () => {
        setSelectedPistolet(null);
        setIsIndexOuvertureModalOpen(false);
    };

    // --- Logique de Filtrage (Cuves) ---
    const filteredStocks = useMemo(() => {
        ...
        return stocks.filter((stock) => {
          ...
          if (statusFilter === 'ecart_negatif') matchesStatus = discrepancy < -0.5;
    ...
    const groupedStocks = useMemo(() => {
        ...
        filteredStocks.forEach(stock => {
            ...
            if (!groups[articleName]) { groups[articleName] = []; }
    ...
    // --- Filtrage des Pompes (Nouvel onglet) ---
    const filteredPompes = useMemo(() => {
        if(!pompes) return [];
        return pompes.filter((pompe) => {
            const pompeName = pompe.name?.toLowerCase() || '';
            const agencyId = pompe.agency_id?.toString() || '';
            const matchesSearch = pompeName.includes(searchTerm.toLowerCase());
            const matchesAgency = selectedAgency === 'all' || agencyId === selectedAgency;
            return matchesSearch && matchesAgency;
        });
    }, [pompes, searchTerm, selectedAgency]);
    ...
    return (
        <>
            ...
            <div className="p-6">
                ...
                <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-3">
                            ...
                            Exploitation Carburant
                        </h1>
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                            Gérez vos stocks en cuve et relevez les compteurs de distribution.
                        </p>
                    </div>
                </div>
                ...
                {/* Filtres & Onglets */}
                <div className="mb-8 rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-800">
                  
                  {/* Les Onglets */}
                  <div className="flex border-b border-gray-200 dark:border-gray-700 px-4">
                      <button 
                        onClick={() => setActiveTab('cuves')}
                        className={`py-4 px-4 font-medium text-sm flex items-center gap-2 border-b-2 transition-colors ${activeTab === 'cuves' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300'}`}
                      >
                      ...
                      <button 
                        ...
                        className={`py-4 px-4 font-medium text-sm flex items-center gap-2 border-b-2 transition-colors ${activeTab === 'pompes' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300'}`}
                      >
                  ...
                  <div className="p-4 flex flex-col gap-4 lg:flex-row lg:items-center">
                    <div className="relative flex-1">
                      ...
                      <input type="text" placeholder={activeTab === 'cuves' ? "Rechercher une cuve..." : "Rechercher une pompe..."} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
                ...
                {activeTab === 'cuves' ? (
                    // ONGLET 1 : CUVES (Ancien code conservé intact)
                    Object.keys(groupedStocks).length > 0 ? (
                            ...
                            {Object.entries(groupedStocks).map(([articleName, groupStocks]) => {
                                ...
                                return (
                                    <div key={articleName} className="rounded-xl border border-gray-200 bg-gray-50 overflow-hidden dark:border-gray-700 dark:bg-gray-900/50">
                                        <div onClick={() => toggleGroup(articleName)} className="flex cursor-pointer items-center justify-between p-4 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <h2 className="font-bold text-gray-800 dark:text-white text-md flex items-center gap-2">{articleName}</h2>
                                                    <p className="text-xs text-gray-500">{groupStocks.length} cuve(s) associée(s)</p>
                                                </div>
                                            </div>
                                            <div className="text-gray-400"><FontAwesomeIcon icon={isExpanded ? faChevronDown : faChevronRight} /></div>
                                        </div>
                                                    ...
                                                    {groupStocks.map((stock) => {
                                                        ...
                                                        return (
                                                            <div key={stock.id} className="relative group flex flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm transition-all hover:shadow-lg dark:border-gray-700 dark:bg-gray-800">
                                                                ...
                                                                <div className="p-5">
                                                                    <div className="mb-2">
                                                                        <h2 className="text-lg font-bold text-gray-800 dark:text-white truncate" title={cuveName}>{cuveName}</h2>
                                                                        <div className="flex items-center text-xs text-gray-500 dark:text-gray-400 mt-1">
                                                                            <FontAwesomeIcon icon={faWarehouse} className="mr-1.5 opacity-70" />{agencyName}
                                                                        </div>
                                                                    </div>
    
                                                                    <div className="my-4">
                                                                        <TankLevelVisualizer quantity={actualQuantity} maxCapacity={maxCapacityL} label={articleName} />
                                                                    </div>
                                                                    ...
                                                                    <div className="bg-gray-50 dark:bg-gray-700/30 rounded-xl p-3 mb-4 border border-gray-100 dark:border-gray-600">
                                                                        <div className="flex justify-between items-center text-xs mb-1">
                                                                            ...
                                                                            <span className="font-mono font-semibold text-gray-700 dark:text-gray-200">{theoreticalQuantity.toLocaleString('fr-FR')} L</span>
                                                                        </div>
                                                                        <div className="w-full bg-gray-200 dark:bg-gray-600 rounded-full h-1.5 mb-2">
                                                                            <div className="bg-gray-400 h-1.5 rounded-full" style={{ width: `${Math.min(100, (theoreticalQuantity/maxCapacityL)*100)}%` }}></div>
                                                                        </div>
                                                                    ...
                                                                    <div className="mt-auto">
                                                                        {isIotConnected ? (
                                                                            <div className="flex flex-col items-center justify-center p-3 text-center bg-blue-50 dark:bg-blue-900/10 rounded-xl border border-blue-100 dark:border-blue-800/30">
                                                                                ...
                                                                                <p className="text-xs font-semibold text-blue-700 dark:text-blue-300">Lecture Automatique</p>
                                                                            </div>
                                                                        ) : (
                                                                            <div className="grid grid-cols-2 gap-3">
                                                                                <button onClick={() => openEditStockModal(stock, 'actual')} className="flex items-center justify-center gap-2 rounded-lg bg-blue-600 px-3 py-2 text-xs font-medium text-white transition hover:bg-blue-700">
                                                                                    <FontAwesomeIcon icon={faClipboardCheck} /> Jauger
                                                                                </button>
                                                                                <button onClick={() => openEditStockModal(stock, 'theoretical')} className="flex items-center justify-center gap-2 rounded-lg bg-purple-600 px-3 py-2 text-xs font-medium text-white transition hover:bg-purple-700">
                                                                                    <FontAwesomeIcon icon={faEdit} /> Corriger
                                                                                </button>
                                                                            </div>
                                                                        )}
                                                                    </div>
                        ...
                        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-gray-300 bg-gray-50 py-16 text-center dark:border-gray-700 dark:bg-gray-800/50">
                            <div className="mb-4 rounded-full bg-gray-100 p-4 dark:bg-gray-800"><FontAwesomeIcon icon={faSearch} className="h-8 w-8 text-gray-400" /></div>
                            <h3 className="text-lg font-medium text-gray-900 dark:text-white">Aucune cuve trouvée</h3>
                            <p className="mt-1 text-gray-500 dark:text-gray-400">Essayez de modifier vos filtres.</p>
                        </div>
                    )
                ) : (
                    // ONGLET 2 : POMPES ET PISTOLETS (Nouveau code)
                    filteredPompes.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 animate-fadeIn">
                            {filteredPompes.map((pompe) => (
                                <div key={pompe.id} className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden flex flex-col dark:border-gray-700 dark:bg-gray-800">
                                    <div className="p-4 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50 flex items-center gap-3">
                                        ...
                                        <div>
                                            <h3 className="font-bold text-gray-800 dark:text-white">{pompe.name}</h3>
                                        </div>
                                    </div>
                                    
                                    <div className="p-4 flex-1 flex flex-col gap-3">
                                        <div className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">
                                            Pistolets & Index ({pompe.pistolets?.length || 0})
                                        </div>
                                        
                                        {pompe.pistolets && pompe.pistolets.length > 0 ? (
                                            pompe.pistolets.map(pistolet => (
                                                <div key={pistolet.id} className="p-3 rounded-lg border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-sm relative group">
                                                    <div className="flex justify-between items-start mb-2">
                                                        <div className="flex items-center gap-2">
                                                            <div className="w-2 h-2 rounded-full bg-green-500"></div>
                                                            <span className="font-bold text-sm text-gray-800 dark:text-gray-200">{pistolet.name}</span>
                                                        </div>
                                                        <span className="text-[10px] bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-600">
                                                            {pistolet.citerne?.article?.name || 'Aucun produit'}
                                                        </span>
                                                    </div>
                                                    
                                                    <div className="flex flex-col bg-gray-50 dark:bg-gray-800 rounded p-2 border border-gray-100 dark:border-gray-700 mt-2">
                                                        <span className="text-[10px] text-gray-500 mb-1">Index Actuel :</span>
                                                        <span className="font-mono font-bold text-lg text-blue-600 dark:text-blue-400">
                                                            {Number(pistolet.current_index).toFixed(2)}
                                                        </span>
                                                    </div>

                                                    <button 
                                                        onClick={() => openIndexOuvertureModal(pistolet)}
                                                        className="mt-2 w-full flex items-center justify-center gap-2 py-1.5 rounded bg-white border border-gray-200 text-xs font-medium text-gray-600 hover:bg-gray-50 hover:text-blue-600 dark:bg-gray-800 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-700 transition-colors"
                                                    >
                                                        <FontAwesomeIcon icon={faEdit} /> Ajuster l'index
                                                    </button>
                                                </div>
                                            ))
                                        ) : (
                                            <div className="flex flex-col items-center justify-center py-4 text-center text-gray-400 bg-gray-50 dark:bg-gray-800/50 rounded border border-dashed border-gray-200 dark:border-gray-700">
                                                <FontAwesomeIcon icon={faPlug} className="mb-2 opacity-50" />
                                                <span className="text-xs">Aucun pistolet installé sur cet îlot.</span>
                                            </div>
                                        )}
                                    </div>
                                </div>
                        ...
                        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-gray-300 bg-gray-50 py-16 text-center dark:border-gray-700 dark:bg-gray-800/50 animate-fadeIn">
                            <div className="mb-4 rounded-full bg-gray-100 p-4 dark:bg-gray-800"><FontAwesomeIcon icon={faSearch} className="h-8 w-8 text-gray-400" /></div>
                            <h3 className="text-lg font-medium text-gray-900 dark:text-white">Aucun Îlot trouvé</h3>
                            <p className="mt-1 text-gray-500 dark:text-gray-400">Vérifiez vos filtres ou contactez l'administrateur pour configurer les pompes.</p>
                        </div>
                    )
                )}
            ...
            {/* NOUVELLE MODALE POUR LES INDEX D'OUVERTURE */}
            <IndexOuvertureModal 
                isOpen={isIndexOuvertureModalOpen}
                onClose={closeIndexOuvertureModal}
                pistolet={selectedPistolet}
            />
        </>
    );
};
