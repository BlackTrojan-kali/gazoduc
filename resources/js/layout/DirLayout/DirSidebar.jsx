...
import { 
  faChevronDown, faCoins, faDashboard, faDotCircle, 
  faDriversLicense, faIndustry, faMoneyBill, faSave, 
  faTimeline, faUser, faUserFriends, faGasPump 
...
const navItems = [
  ...
  },
  {
    ...
    name: "Cloture Comptes",
    subItems: [
      { name:"Clotures", path: "/closures/index", pro: false },
      // NOUVEAU MODULE AJOUTÉ ICI :
      { name:"Déclaration CSPH", path: "/direction/csph", pro: false },
    ],
  },
  {
    ...
    >
      <div className={`py-8 flex ${!isExpanded && !isHovered ? "lg:justify-center" : "justify-start"}`}>
        <Link href="/">
          {isExpanded || isHovered || isMobileOpen ? (
            <>
              <img className="dark:hidden" src={"/images/clients/"+auth.user.entreprice.logo_path} alt="Logo" width={80} height={40} />
              <img className="hidden dark:block" src={"/images/clients/"+auth.user.entreprice.logo_path} alt="Logo" width={80} height={40} />
            </>
          ) : (
            <img src={"/images/clients/"+auth.user.entreprice.logo_path} alt="Logo" width={32} height={32} />
          )}
        </Link>
      </div>
          ...
          <div className="flex flex-col gap-4">
            <div>
              ...
              <h2 className={`mb-4 text-xs uppercase flex leading-[20px] text-gray-400 ${!isExpanded && !isHovered ? "lg:justify-center" : "justify-start"}`}>
                {isExpanded || isHovered || isMobileOpen ? "Menu" : <FontAwesomeIcon icon={faDotCircle} className="size-6" />}
              </h2>
              {renderMenuItems(navItems, "main")}
            </div>
            <div className="">
              <h2 className={`mb-4 text-xs uppercase flex leading-[20px] text-gray-400 ${!isExpanded && !isHovered ? "lg:justify-center" : "justify-start"}`}>
                {isExpanded || isHovered || isMobileOpen ? "Users" : <FontAwesomeIcon icon={faDotCircle} />}
              </h2>
